...
2. 文字列を`list` に
3. `list` の長さで`for` で回す。`Token`化し、文字列分index を進める
    1. 各型に振り分け
        - `symbols`
            - `[` `{` `:` `,` とか
        - `bool` `null`
...
3. `Token` のリストを返す
4. `[`, `{`, `]`, `}` を検知して、`nest` を設定
5. `:` を検知して、index の`-1` を`obj_key` に設定
6. `nest` 設定されている`Token` の開始と終了のindex を取得
7. `nest` の深さと合わせて`list` で返す
8. `nest` の終始に合わせて、間の`Token` に、`indent` を付与
    - 入れ子構造になっている内部の深さを指定してる
    - 深さが上がる場合に次の要素でしか判断できない
        - ex: 深さが5 ある場合には、5 の深さの`Token` は、5回書き換えがある
9. 深さに合わせて、各々のオブジェクトをつくる
